...
require_once('../models/handler/rol_handler.php');
/*
 *  Clase para manejar el encapsulamiento de los datos de la tabla USUARIO.
 */
class rolesData extends RolHandler
{
    ...
    public function setNombre($value, $min = 2, $max = 50)
    {
        ...
        $checkSql = 'SELECT COUNT(*) as count FROM TB_ROLES WHERE nombre_rol = ?';
        $checkParams = array($value);
        $checkResult = Database::getRow($checkSql, $checkParams);
    
        if ($checkResult['count'] > 0) {
            $this->data_error = 'El rol ya existe';
            return false;
        }
